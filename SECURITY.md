...
|-------|----------------|----------------|---------------|---------------------------|
| `sha3` (RustCrypto) | pending first-use | None in pure-Rust mode; SIMD backends use `unsafe` for vectorised intrinsics. | Hardware-accelerated SHA3 on x86-64 and ARM v8.4-A matters for consensus-critical hashing throughput. | Widely deployed across the RustCrypto ecosystem; routine community review. |
