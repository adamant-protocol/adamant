...
The active validator set is dynamic, with a constitutional floor of 7 validators (the minimum size at which Byzantine fault tolerance retains non-zero margin against correlated failures) and a soft ceiling of 75 validators at launch (calibrated to the residential-fibre hardware profile of the launch period). Selection follows a first-come-first-served-with-transferable-membership rule: validators are admitted to the active set in registration order, retain their slot through continuous participation, and may transfer their slot to another address by mutual consent. The launch period rewards registration order and continuity; long-term active-set composition reflects the choices of slot holders and is expected to evolve as the chain matures. The chain self-activates the moment 7 validators are simultaneously registered, stake-eligible, and online; there is no coordination event, no recruited genesis cohort, no foundation. The first 75 validators to take active-set slots form the **genesis cohort** and receive a permanent, non-transferable on-chain marker plus a freely tradeable Genesis NFT recognising their role in bootstrapping the network. Below the floor, the chain halts rather than forks, preserving safety at the cost of liveness during periods of severe validator unavailability. The chain commits an on-chain security-tier signal (Tier I at 7–14 validators, Tier II at 15–29, Tier III at 30+) so that wallets and applications can adapt to the chain's current security posture rather than assume the design-target posture from launch onward.

Participation is structured across four tiers with bounded power. **Validators (Node Runners)** do consensus and mempool decryption; minimum stake 1,000 ADM at launch. **Witnesses (Node Watchers)** perform attestation, data availability sampling, recursive proof verification, and fraud detection on phone-class hardware; minimum stake 100 ADM at launch. **Provers** do steady-state recursive-proof generation in a permissionless market, paid per-proof from transaction fees; validators do proof generation as a fallback at degraded cadence to preserve phone-verifiability regardless of prover-market health. **Service nodes** provide light-client infrastructure. The role split keeps validator hardware on residential-fibre commodity desktops at launch by moving GPU-class proof generation off-tier; phone-class participation is meaningful via witnesses; no single tier alone controls the chain. Stake parameters are subject to scheduled re-evaluation at five-year intervals post-genesis (subsection 11.5.4) to prevent token-appreciation-driven exclusion.
...
Adamant's throughput floor at launch is 50,000+ TPS at the launch-period active-set ceiling rather than the 200,000+ TPS demonstrated by the Mysticeti / Sui line. This is a deliberate choice for the bootstrap composition of the chain: a 200,000 TPS target on a DAG-BFT mechanism (where communication cost grows quadratically in validator count) implies an active set of approximately 200 validators, which in turn implies VPS-grade hardware at every validator. That hardware tier excludes the residential-fibre operators the protocol is designed to *bootstrap with*. A 50,000 TPS floor tolerates an active-set ceiling of 75, which is feasible on consumer-desktop hardware on residential fibre. The 50,000 figure is a floor rather than a cap: actual throughput depends on the active set's aggregate hardware and network conditions, and routinely exceeds the floor when validators run better-than-baseline hardware. As the chain's hardware composition evolves over time, the ceiling and throughput floor may be revised upward via hard fork (subsection 8.1.10). 50,000 TPS at launch remains substantially above the sustained throughput of any production L1 in 2026 and is an order of magnitude above the requirements of any consumer payment workload. The figure is subject to empirical validation on residential-fibre hardware before genesis.
...
The active validator set is dynamic. The constitutional floor is 7 validators — the smallest size at which Byzantine fault tolerance retains non-zero margin against correlated failures (at N=7 the chain tolerates f=2 Byzantine validators, so one Byzantine validator combined with one offline validator still leaves the chain inside its safety bound). The soft ceiling at launch is 75 validators, calibrated to the residential-fibre hardware profile of the launch period. Selection follows a first-come-first-served-with-transferable-membership rule: validators are admitted in registration order, retain their slot continuously through participation, and may transfer it to another address by mutual consent. The launch period rewards registration order and continuity; the long-term composition of the active set is determined by the choices of slot holders. The chain self-activates at the floor: block production begins the moment 7 validators are simultaneously registered, stake-eligible, and online, with no coordination event and no human-in-the-loop activation. Below the floor, the chain halts on disagreement rather than forking, preserving safety at the cost of liveness during periods of severe validator unavailability. The first 75 validators to take active-set slots form the **genesis cohort** and receive a permanent on-chain marker plus a tradeable Genesis NFT recognising their role in bootstrapping the chain.
...
The 50,000 TPS floor is calibrated to the throughput at which DAG-BFT communication cost (O(N²) in the active set size) is operationally feasible on residential-fibre validator hardware at the launch active-set ceiling of 75 validators. Earlier drafts of this whitepaper specified a 200,000 TPS target derived from the demonstrated throughput of Mysticeti consensus on VPS-grade validator hardware; the launch-period target was reduced because the chain commits to bootstrapping on residential-fibre hardware rather than on datacenter infrastructure. 50,000 TPS is roughly 30x Visa's average global throughput, matches or exceeds the sustained throughput of contemporary high-performance L1s, and is genuinely state-of-the-art for a privacy-default chain. The empirical validation caveat is necessary because the sizing argument from 200k @ 200 validators to 50k @ 75 validators is provisional; section 8 specifies the calculation and the validation requirement.
...
**Launch-period vs. long-term hardware composition.** The residential-fibre profile is a property of the chain's launch composition, not a permanent structural commitment. Validator slots are transferable between addresses by mutual consent (subsection 8.1.8), and over time the active set is expected to evolve toward whatever hardware tier its operators choose to run. The whitepaper commits to launching on residential-fibre hardware so that the first 75 validators can be ordinary participants rather than infrastructure operators; it does not commit to permanently constraining the chain to that hardware tier. As the active set's hardware composition concentrates toward higher tiers, the active-set ceiling and throughput floor may be revised upward via hard fork (subsection 8.1.10) to reflect the chain's actual capability. The launch period is the chain's home-runnable period; the chain's long-term character is determined by the choices of slot holders and the social-coordination process that governs parameter revisions.
...
In any epoch, a subset of registered validators forms the **active set** — the validators currently responsible for consensus. The active set is dynamic, with a constitutional floor and a soft ceiling. Membership is acquired by registration order, retained through continuous participation, and may be transferred between addresses by mutual consent (subsection 8.1.6).
...
**Soft ceiling: 75 validators at launch.** This number is calibrated to the residential-fibre profile of the chain's launch period. DAG-BFT communication cost grows quadratically in active-set size; per-validator bandwidth load grows linearly. At the 50,000 TPS minimum-throughput floor on residential-fibre hardware (commodity desktop, 1 Gbps fibre, ~100 ms typical wide-area latency), 75 active validators is the upper bound at which the per-validator bandwidth and verification cost remain tractable without exceeding the residential-fibre profile at launch. The ceiling is explicitly a **soft ceiling, subject to revision** as the chain's hardware composition evolves over time (subsection 8.1.7). The exact number at launch is subject to empirical validation prior to mainnet (subsection 8.10).

**Selection: first-come-first-served with transferable membership.** When the count of registered, stake-eligible, currently-online validators is at or below the ceiling, every such validator is in the active set. When the count exceeds the ceiling, validators are admitted in **registration order**: the first 75 to register and meet the eligibility criteria fill the active set, and subsequent registrants enter a standby queue. A validator's active-set slot is held continuously as long as the validator continues to participate. The slot is released only when (a) the validator is removed for liveness failure (subsection 8.1.5: failure to participate for more than 2 consecutive epochs while in the active set), (b) the validator voluntarily unbonds, or (c) the validator transfers the slot to another address (subsection 8.1.6). When a slot opens through any of these mechanisms, the next standby validator in queue order is admitted to the active set automatically at the next epoch boundary. There is no forced rotation: a validator who registered early and continues to show up retains their slot indefinitely until they choose to exit or transfer.

**Why first-come-first-served at launch.** This selection mechanism rewards *registration order and continuity* during the launch period rather than hardware budget or stake size. A small home-fibre validator who registered on day 3 of the chain cannot be displaced by a wealthier latecomer through stake competition or by a faster competitor through performance pressure — the mechanism prevents involuntary displacement. Stake-weighted-lottery and performance-tier selection mechanisms — both standard elsewhere — would, given enough time, push the active set toward whichever validators have the deepest hardware budgets, eroding the home-runnable property through selection pressure. First-come-first-served avoids that drift through the launch period. Voluntary slot transfers (subsection 8.1.8) provide an exit pathway for validators who choose to realise their position; the chain does not constrain who they choose to transfer to.
...
**Sizing rationale at launch.** The relationship between active-set size N, throughput, and hardware tier is roughly:
...
The 75 ceiling at 50,000 TPS reflects the chain's launch-period commitment to residential-fibre hardware. As the chain's hardware composition evolves over time (subsection 8.1.7), the ceiling and throughput floor may be revised upward to reflect the actual capability of the active set.

**Throughput as a floor, not a target.** The 50,000 TPS figure is the chain's *minimum commitment* at the launch active-set ceiling, not a cap. Actual throughput in operation depends on the active set's aggregate hardware capability and current network conditions, and routinely exceeds the floor when validators run better-than-baseline hardware or when network conditions are favourable. The chain commits to delivering at least 50,000 TPS at N=75 on baseline residential fibre; the reality often delivers more. The figure is subject to empirical validation prior to mainnet.
...
A validator who holds an active-set slot may transfer it to another address by mutual consent. Transfer is implemented at the protocol layer and produces an atomic state transition: the seller's slot status, stake, and delegations are released; the buyer's address takes the slot, posts the current minimum stake, and continues validation from the next epoch boundary without consensus interruption.

**Mechanism.** The seller initiates a `transfer_slot(buyer_address)` transaction, signed by the seller's validator key. The buyer must co-sign the transaction with their address's signing key, demonstrating consent and capability to fulfil the validator role. At the next epoch boundary, the protocol:

1. Releases the seller's bonded validator stake to their address (subject to the standard 28-day unbonding period for slashing accountability against any pre-transfer offences)
2. Bonds the current minimum validator stake (1,000 ADM at launch, subject to subsection 11.5.4) from the buyer's address
3. Reassigns the slot to the buyer's address with no change in slot ordering or seniority within the active set
4. Migrates any active delegations to the seller (delegators have a 7-day window post-transfer to re-delegate to a different validator if they object to the new operator; during this window, delegations remain bonded but earn no rewards)

**No protocol-imposed price or fee.** The chain does not set a price for slot transfers, does not collect a fee on the transfer, and does not constrain who the buyer may be. The seller and buyer negotiate compensation off-chain or via on-chain auction mechanisms built atop the transfer primitive. The protocol is neutral on the transaction's economic terms; it enforces only the technical correctness of the handoff.

**No protocol-imposed counterparty restriction.** The buyer may be any address — an individual, a fund, a syndicate, an exchange, a smart-contract wallet. The protocol does not perform identity checks, does not maintain whitelists or blacklists, and does not have the capability to do so. Slot ownership reflects whoever the seller chose to transfer to.

**Consequences.** Slot transferability is a deliberate design choice with structural consequences for the chain's long-term character. Over time, slots are likely to accumulate with whoever can offer the most for them, which creates a path toward concentration in operators with deeper capital. The launch period's home-runnable property is therefore a property of the launch composition, not a permanent structural commitment of the protocol. The chain accepts this trade in exchange for a strong launch incentive: early validators capture the upside of having bootstrapped the chain, and that captured upside is expected to be a meaningful recruitment mechanism for the genesis cohort.

**Genesis cohort marker does not transfer with the slot** (subsection 8.1.9). When a genesis-cohort validator transfers their slot, the slot moves to the buyer; the genesis cohort marker remains attached to the seller's original address. The buyer takes operational control of consensus from the slot; they do not inherit the historical recognition of having bootstrapped the chain.

### 8.1.9 Genesis cohort

The first 75 validator addresses to register and successfully take an active-set slot during the chain's launch period are the **genesis cohort**. The chain commits two on-chain artefacts to recognise their role in bootstrapping the network.

**Genesis cohort marker.** A permanent, non-transferable on-chain attribute is attached to each of the first 75 validator addresses at the moment they take their active-set slot. The marker records the address's position in the genesis cohort (1 through 75), the epoch at which they were activated, and a cryptographic commitment to the chain state at that moment. The marker remains attached to the original address for the lifetime of the chain. It does not transfer with slot transfers, cannot be moved between addresses, and cannot be revoked. A genesis-cohort validator who later sells their slot, exits, or is slashed retains the marker on their address — it records who they were at the moment of activation, not what they later did.

**Genesis NFT.** A non-fungible token is minted to each genesis cohort validator's address at the moment they take their slot. The NFT is freely transferable and tradeable as a cultural artefact: it has no protocol-level function, conveys no rights or privileges, and does not affect consensus participation. Holders of a Genesis NFT are not validators by virtue of holding the NFT; they are owners of a historical token that records the original validator's participation in the genesis cohort. A genesis validator may sell their NFT, hold it, or transfer it to family or successors. The NFT exists as a way for the original cohort to capture cultural value from having bootstrapped the chain, separable from any decision they later make about the slot itself.

**Why both.** The two artefacts serve different purposes and are intentionally separable. The marker is non-transferable and permanent because it records a historical fact about an address — the chain's record of who was there, which is not a property anyone can sell or transfer. The NFT is transferable because it is a cultural artefact, and cultural artefacts are properly tradeable. Genesis validators who later transfer their slots retain the marker (the chain remembers them) and may keep, sell, or pass on the NFT (the cultural value of having been there).

**Closure.** The genesis cohort is closed once 75 distinct validator addresses have taken active-set slots during the launch period. Slot 76 onward is a regular validator slot, with no marker and no NFT. If a genesis-cohort validator is later removed for liveness failure or transfers their slot, the slot becomes a regular slot (the next standby validator who takes it does not become a member of the genesis cohort). The cohort is defined by the chain's first 75 active-set assignments, not by current slot ownership.

### 8.1.10 Future revision

The active-set ceiling of 75 is a soft ceiling calibrated to the residential-fibre profile of the chain's launch period. The ceiling is explicitly subject to revision via hard fork as the chain's hardware composition, network conditions, and consensus implementation maturity evolve. As validator hardware concentrates over time (a likely consequence of slot transferability per subsection 8.1.8), the per-validator hardware capability of the active set may rise to a level supporting a higher ceiling and a higher throughput floor. Such revisions follow the standard hard-fork mechanism (subsection 11.4) and are not anticipated to occur before year 3 of operation.

The floor of 7 is a constitutional minimum and not subject to revision unless the BFT mathematics that justify it changes.

The throughput floor of 50,000 TPS is calibrated to the launch ceiling and the residential-fibre hardware profile. Revisions to the active-set ceiling that reflect concentration toward higher hardware tiers may be accompanied by revisions to the throughput floor in the same fork, reflecting the actual capability of the post-revision active set.
