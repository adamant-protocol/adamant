...
#### 6.2.1.8 Module deserializer and verifier-projection architecture

Sections 6.2.1.1 through 6.2.1.7 describe the bytecode language. This subsection describes how a conforming Adamant implementation parses bytecode bytes into an in-memory module representation, runs verification over that representation, and divides verification work between the inherited Sui-Move passes (which operate on the Sui-base subset) and the Adamant-specific verifier rules (which operate on the full Adamant superset including extensions).

**The integration problem.** Section 6.2.1.1 commits Adamant to inheriting Sui-Move's binary format and verifier; the vendored `move-binary-format` and `move-bytecode-verifier` crates are the authoritative reference for that inheritance. But the inheritance is at the *language* level — the vendored Sui crates implement only the Sui-base subset. Specifically:

- Sui-Move's deserializer (`CompiledModule::deserialize_with_config`) dispatches over a closed match of opcode bytes covering Sui's instruction set; bytes outside that range are rejected with `UNKNOWN_OPCODE`. Adamant extension opcodes (in the reserved range above Sui-Move's last opcode per section 6.2.1.4) fall outside Sui's match and would be rejected by Sui's deserializer.
- Sui-Move's verifier (`move-bytecode-verifier`) operates on Sui's `Bytecode` enum, which has no representation for Adamant extensions. Sui's per-instruction passes (instruction consistency, control-flow, stack usage, type safety, locals safety, reference safety) are exhaustive matches over Sui's `Bytecode`; an Adamant extension instruction has no defined behaviour in those passes.

Both gaps are properties of the vendored crates, not of the spec, and both must be addressed by an Adamant-native parsing-and-verification pipeline that wraps the vendored Sui logic where it applies and adds Adamant-specific logic where it does not.

**The Adamant-native deserializer.** A conforming Adamant implementation provides a deserializer that parses module bytes into an Adamant module representation containing the full Adamant instruction set. The representation extends Sui-Move's `CompiledModule` shape — same module-level pools, same handle tables, same per-function metadata — with one structural change: function bodies are sequences over the Adamant instruction set (Sui-base instructions plus Adamant extensions) rather than over Sui-Move's `Bytecode` enum alone. The wire encoding for function bodies is specified in section 6.2.1.5; the Adamant-native deserializer delegates per-instruction parsing of inherited Sui-Move opcodes to the vendored Sui parsing logic and parses Adamant extension opcodes via the wire-encoding layer. Module-level structure (handle tables, signature pool, identifiers, addresses, constants, metadata, struct definitions, function definition headers) is parsed via the vendored Sui parsing logic unchanged — only function bodies require the extended dispatch.
...
**The Sui-projection mechanism.** Sui-Move's verifier passes establish properties Adamant inherits — type safety, reference safety, linearity (copy/drop semantics), operand-stack discipline, control-flow integrity (CFG reducibility, branch-target validity), generic-instantiation consistency, and friend visibility — over the Sui-base subset. To run those passes against an Adamant module, the Adamant-native verifier driver constructs a *Sui projection*: a Sui-Move `CompiledModule` whose function bodies are the original module's function bodies with each Adamant extension instruction substituted by Sui-Move's `Nop` instruction. The projection preserves positional indices in function bodies (substitution is one-for-one), so branch targets — which are positional offsets into the function's instruction array — remain valid without rewriting. Sui's verifier passes run against the projection and establish their guarantees over the Sui-base subset of each function's body; the projection is discarded after verification and plays no role in execution.

The substitution choice — `Nop` rather than instruction stripping or per-function exclusion — is structural. Stripping (removing Adamant extensions and shifting subsequent instructions) would invalidate every branch target whose offset crossed a removed instruction, requiring per-instruction offset rewriting that is consensus-critical risk surface (a mis-rewritten target produces a different CFG and different verifier results across implementations). Per-function exclusion (skipping Sui's verifier on functions containing any Adamant extension) would surrender Sui's per-instruction guarantees on exactly the functions most worth verifying — privacy-using functions, which are the highest-value targets for verification. `Nop` substitution preserves indices one-for-one, uses a Sui-native instruction that is already idiomatic in Sui's own test fixtures and stress harnesses, and lets every Sui-base instruction in an Adamant function be verified normally with Adamant extensions appearing as benign no-ops in the projection.

**What Sui's verifier proves on the projection.** Sui's per-instruction passes establish their guarantees over the Sui-base instructions in the projection. Specifically, for the Sui-base subset of each function's body, the projection-verified module satisfies:

- *Type safety:* every operand on the stack at the consumption point of a Sui-base instruction has the type that instruction expects.
- *Reference safety:* borrowed references created by Sui-base instructions do not outlive the values they reference; mutable references are not aliased.
- *Linearity:* values without the `copy` ability are not duplicated by Sui-base instructions; values without the `drop` ability are not implicitly discarded by Sui-base instructions.
- *Stack discipline:* the operand stack depth at every basic-block boundary is statically bounded; no Sui-base instruction underflows the stack or causes runaway growth.
- *Control-flow integrity:* every branch target is a valid instruction within the function; the CFG is reducible per Sui's `loop_summary` analysis.
- *Generic instantiation:* type arguments to Sui-base generic instructions satisfy the abilities required by the generic parameters.
- *Friend visibility:* friend calls in Sui-base `Call` and `CallGeneric` instructions only reach declared friends.

**What Sui's verifier does not prove.** Adamant extensions appear as `Nop` in the projection. Sui's passes therefore do not establish type safety, reference safety, or stack discipline for the Adamant extensions themselves. The per-instruction stack effects, operand types, and reference behaviour of the extensions specified in section 6.2.1.4 are not verified by Sui's passes against the projection.

Adamant extensions are verified by two complementary mechanisms:

- *Adamant-specific verifier rules (section 6.2.1.6).* These rules examine the unmodified Adamant module representation (not the projection) and enforce structural and semantic constraints — privacy consistency across the call graph, privacy-circuit instructions only in shielded contexts, dynamic-dispatch restrictions, and the other rules enumerated in section 6.2.1.6.
- *AVM runtime (section 6.2.2).* The runtime carries the consensus-binding semantics of every extension instruction: stack effects, type checks, and abort conditions are enforced dynamically per section 6.2.2's execution model. Per section 6.2.4's determinism guarantee, runtime behaviour is determined entirely by the inputs to the AVM and the bytecode itself; runtime verification of extension instructions reaches identical accept/reject decisions across all conforming implementations.

A conforming implementation may additionally provide static (deploy-time) per-instruction verification of Adamant extensions — analogous to Sui's per-instruction passes but extended to the Adamant instruction set. Such verification is a strengthening property and may catch errors at deployment that the runtime would otherwise catch at first execution; it does not change the consensus model, since the runtime is the binding mechanism. The minimum bar for conformance is that the Adamant-specific verifier rules of section 6.2.1.6 are enforced at deployment; full per-instruction static verification of extensions is implementation-discretionary.

**Pipeline ordering.** A conforming Adamant deployment validator processes module bytes in the following order:

1. *Adamant-native deserialize.* Parse the bytes into the Adamant module representation. Reject malformed bytes, unknown opcodes (including bytes outside both the Sui-base and Adamant-extension opcode ranges), or any structural violation of the binary format.
...
3. *Sui-projection construction.* Build the Sui projection by substituting each Adamant extension instruction in each function body with `Nop`. Module-level structure is unchanged.
4. *Inherited Sui verifier passes.* Run Sui's `move-bytecode-verifier` against the projection. Reject on any failure (deprecated global-storage instructions per section 6.2.1.6 rule 5, type-safety violations, reference-safety violations, stack-discipline violations, control-flow violations, etc.). The projection is discarded after this step.
5. *Adamant-specific verifier rules.* Run the Adamant-specific rules of section 6.2.1.6 against the unmodified Adamant module representation. Reject on any failure.
...
**Implementation note.** The Adamant-native deserializer and the Sui-projection mechanism are reference-implementation concerns rather than protocol-level specifications: a conforming implementation may parse and verify modules using any internal representation, provided the accept/reject decisions match the spec's pipeline above. Two implementations that disagree on whether a given module-bytes input is valid for deployment are not both conforming, regardless of internal data-structure choices.
