...
**Next sub-arc — Phase 7.7b**: commit-wave logic per §8.3.3 + §8.6. Anchor election uses the §8.6 BLS-aggregate VRF (shipped in Phase 7.4) over the wave's last round; the commit decision walks causal-ancestry to determine which round-anchors achieve the 2/3+1 support necessary to commit; the causal-history walk produces the totally-ordered sequence of vertices to feed the §6 execution layer. Builds directly on the Phase 7.7a `causal_ancestors` + `reaches` helpers and the `vertices_at_round` enumeration accessor.
