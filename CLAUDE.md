...
| 7.7d | §3.6, §3.8, §8.4 | mempool integration (threshold/time-lock decryption flows) | pending |
...
**Next sub-arc — Phase 7.7d**: mempool integration. The committed-wave output (per-wave `ordered` Vec<VertexId>) needs to feed through the §3.6 threshold mempool (when N ≥ 15) or §3.8 time-lock mempool (when N < 15) per the §3.8.5 hysteresis regime tracked by Phase 7.6's `RegimeState`. Per §8.4 the decryption flow varies by regime: threshold mode uses validator-published decryption shares with 2/3+1 aggregation; time-lock mode uses the round anchor's atomic VDF decryption per §8.4.4 Mitigation B. Phase 7.7d ships the decryption-flow types + the §3.6 share-aggregation wiring + the §3.8 anchor-decryption wiring on top of the Phase 7.5 envelope encryption primitives.
