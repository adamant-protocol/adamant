...
| 7.7c | §8.3.3, §8.7 | indirect commit + halt-on-disagreement + §8.7 invariants | pending |
...
**Next sub-arc — Phase 7.7c**: indirect commit rule + halt-on-disagreement + §8.7 safety/liveness invariants. The indirect commit rule layers a `+3`-round look-ahead onto direct: when a wave is undecided at `anchor_round + 2`, skip-votes counted at `anchor_round + 3` decide whether the wave is forcibly skipped or pulled forward via a future wave's anchor. Halt-on-disagreement at the §8.7.1 floor (`N ≥ ACTIVE_SET_FLOOR = 7`): when quorum is unreachable, the chain pauses rather than forks. §8.7 safety/liveness theorem pins (no double-commit; eventually-commits-under-honest-majority).
