...
**Code**: 20 workspace members at present. 7 Adamant-authored crates (`adamant-account`, `adamant-bytecode-format`, `adamant-crypto`, `adamant-crypto-blst-extra`, `adamant-state`, `adamant-types`, `adamant-vm`) plus 13 vendored Sui-Move crates at tag `mainnet-v1.66.2` — Batch 1 (`move-binary-format`, `move-core-types`, `enum-compat-util`, `move-proc-macros`, `move-abstract-interpreter`) and Batch 2 (`move-bytecode-verifier`, `move-borrow-graph`, `move-bytecode-verifier-meter`, `move-vm-config`, `move-abstract-stack`, `move-regex-borrow-graph`, `move-command-line-common`, `move-symbol-pool`). Phase 5/5b.5 will move the 13 vendored Sui-Move crates from runtime dependencies to test-only dev-dependencies of the production-binary target, with a build-system independence check (`tests/no_sui_in_production_deps.rs`) walking the resolved dependency tree via `cargo metadata` and asserting no `move-*` crate appears. **1290 tests passing across the workspace at Phase 5/5b.4 D-1 sub-arc closure** (1259 at Phase 5/5b.3 closure + 31 across D-1a/D-1b). Workspace test count progression across Phase 5/5b.3: **1035 → 1259 (+224)**. Test breakdown at Phase 5/5b.3 closure: `adamant-bytecode-format` 96 lib unit tests + 55 cross-validation tests (unchanged through Phase 5/5b.3); `adamant-vm` test count 358 → 588 across Phase 5/5b.3 + D-1 sub-arc (corrects the prior C-5 state-bump claim of 582 — empirically 557 at Phase 5/5b.3 closure commit `7d90847`; D-1 state-bump uses grep-confirmed empirical baseline per the C-5 commit-message running-total drift discipline; corrigendum to land in `crates/adamant-vm/src/validator/module_pass/PROVENANCE.md` at D-7 closure batch alongside the existing B-6 entry; +199 across Phase 5/5b.3: bounds_checker 162 tests across C-1.1 → C-1.4b; duplication_checker 38 tests at C-2; signature_checker 19 tests at C-3 including 2 from variant-vs-test-mapping audit catch; 5 integration + precedence-parity tests at `validator/mod.rs::tests` from C-4); other workspace crates' test counts unchanged. D-1 sub-arc adds 31 tests in `validator/function_pass/`: 18 CFG construction tests + 12 AI framework tests with synthetic SawPop domain + 1 cfg.instructions accessor test. **Production-dependency posture at Phase 5/5b.3 closure:** `petgraph 0.8.x` remains the only non-Sui-vendor-derived production dep on `adamant-vm` (added at B-3.2; no new external production deps across all of Phase 5/5b.3 per the seven-criterion external-production-dep audit template). Production-dependency posture unchanged at D-1a + D-1b: no new external production deps; D-1 sub-arc is pure infrastructure work. The transitional Sui-verifier bridge in `validator/mod.rs::verify_module` is retained behind `if !module.contains_adamant_extensions()` for inherited-subset modules through Phase 5/5b.4; tears out at 5/5b.5 alongside the production-target dependency-tree independence check. The bridge runs as defense-in-depth alongside the now-complete Adamant-native step-3 batch (11 passes wired at C-4); semantic parity asserted by Layer B cross-validation tests in each pass. **`AdamantValidationError` carries 50 typed variants at Phase 5/5b.3 closure** (empirically grep-confirmed; corrects the prior B-6 state-bump claim of 20 — see corrigendum in `crates/adamant-vm/src/validator/module_pass/PROVENANCE.md`. Pre-Phase-5/5b.3 baseline was 33; Phase 5/5b.3 added 17). D-1a + D-1b ship 0 new typed variants per Q1 walk-back precedent (foundation-then-producer arcs declare variants alongside producer at D-2); count unchanged from Phase 5/5b.3 closure. **5 public closed enums** at Phase 5/5b.3 closure: `MalformedConstantReason` (B-2.1), `FieldOwnerKind` (B-2.3), `HandleKind` (B-3.1), `DefKind` (C-2; `Struct | Enum | Function`), `InvalidSignatureReason` (C-3; `RefInsideContainer | RefAsFieldType`) — all re-exported via `validator/mod.rs`. **5 helpers extracted across Phase 5/5b.3:** `check_index` (C-1.1; generic per-pool bounds-check); `check_module_handle` (C-1.2; per-handle reusing across module-handles + friend-decls); `check_field_def` (C-1.3; per-field reusing across struct + enum field iteration; deliberate-Adamant-decision instance); `check_signature_type_parameters` (C-1.4b; recursion helper for generic-call-site bytecode arms); `check_code_index` (C-1.4b; per-bytecode bounds-check producing `CodeIndexOutOfBounds`). **Variant-vs-test mapping audit principle established at C-3** and applied retroactively across all 50 typed variants at C-5 closure: 49 variants have explicit negative-test coverage; 1 gap surfaced (`SuiVerifier` transitional-bridge variant) deferred to natural resolution at Phase 5/5b.5 bridge tear-out per documented disposition.
