# AGENTS.md — Project briefing for Codex

This file is read automatically at the start of every Codex session in this repository. It is the load-bearing context document for the project. **Read it in full before doing anything in this repository.**
...
1. **`/whitepaper/02-design-principles.md`** — The seven principles in priority order. The most important file in the repo. **Proposals contradicting any principle are rejected on principle, not re-litigated.** If a request to Codex conflicts with a principle, push back on the request rather than silently compromising.
...
The whitepaper is canonical. If implementation reveals a problem with the spec, we update the spec first (in conversation with Ryan in the main Codex chat), then implement. Never the other way around. **Code that conflicts with the whitepaper is buggy code, not a revised spec.**
...
3. Tell Ryan. Suggest he raise it with Codex in the main chat for a spec revision.
...
- Modify the whitepaper without explicit user approval. The spec is constitutional. If the user asks you to update the spec, suggest they do it in the main Codex chat where the spec is being maintained — Codex is for implementation.
- **Never modify `/whitepaper/02-design-principles.md` or `/whitepaper/11-genesis-constitution.md`.** These are the load-bearing constitutional sections. Any change requires deliberate process, not a Codex session.
