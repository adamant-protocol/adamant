...
pub fn select_index(randomness: &[u8; VRF_RANDOMNESS_BYTES], n: usize) -> usize {
    ...
    // `as usize` is safe on 64-bit platforms; for 32-bit
    // platforms the cast truncates the high bits but the modular
    // reduction maintains uniformity in expectation.
    #[allow(
        clippy::cast_possible_truncation,
        reason = "modular reduction preserves uniformity"
    )]
    {
        (r % n as u64) as usize
    }
