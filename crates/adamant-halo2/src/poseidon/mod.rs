//! The Poseidon algebraic hash function (in-circuit chip + the
//! out-of-circuit primitives module).
//!
//! Combined fork of upstream's `halo2_gadgets::poseidon` (chip
//! surface, this file + `pow5.rs`) and `halo2_poseidon` (out-of-
//! circuit primitives, the [`primitives`] sub-module). Adamant's
//! Phase 6.8b.0 vendored the primitives; Phase 6.8b.2 adds the
//! chip surface alongside.
//!
//! Behavioural changes from upstream (see
//! `crates/adamant-halo2/PROVENANCE.md` sub-arc 6.8b.2):
//!
//! 1. `pub use ::halo2_poseidon as primitives;` (upstream
//!    re-exports the external `halo2_poseidon` crate at the
//!    `primitives` path) replaced with `pub mod primitives;`
//!    pointing at the Adamant-owned fork of those primitives.
//! 2. `halo2_proofs::*` paths rewritten to
//!    `crate::proofs::*` to refer to Adamant's fork of
//!    `halo2_proofs` rather than the upstream crate.
