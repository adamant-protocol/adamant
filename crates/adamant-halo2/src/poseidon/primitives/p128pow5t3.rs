// `Vec` resolves from `std`'s prelude post-fork (upstream is
// `no_std + alloc`; Adamant's `adamant-halo2` is `std`).
use ff::Field;
...
#[cfg(test)]
mod tests {
    // `Vec` resolves from `std`'s prelude post-fork.
    ...
    // Path adjusted post-fork: upstream `halo2_poseidon` had
    // these at the crate root (its `lib.rs`); in `adamant-halo2`
    // the same items live one module level up under
    // `crate::poseidon::*`, reachable here via `super::super::*`.
    use super::super::{generate_constants, permute, ConstantLength, Hash, Mds, Spec};
    ...
    fn permute_test_vectors() {
        {
            ...
            for tv in super::super::test_vectors::fp::permute() {
                let mut state = [
...

        {
            ...
            for tv in super::super::test_vectors::fq::permute() {
                let mut state = [
    ...
    fn hash_test_vectors() {
        for tv in super::super::test_vectors::fp::hash() {
            let message = [
        ...
        for tv in super::super::test_vectors::fq::hash() {
            let message = [
