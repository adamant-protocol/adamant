...
//!
//! Forked from `halo2_poseidon 0.1.0` per CLAUDE.md §14.4
//! Decision 1 (resolved as Path C2). Adamant-owned; refresh
//! cadence and audit posture documented in
//! `crates/adamant-halo2/PROVENANCE.md`. Behavioural changes
//! from upstream are limited to:
//!
//! - `#![no_std]` removed at the module level (the crate root
//!   is `std` by workspace convention; Adamant has no `no_std`
//!   target).
//! - `extern crate alloc;` removed; `String` / `Vec` resolve
//!   from `std`'s prelude / re-exports.
//!
//! No algorithmic deviation. The Poseidon parameter set is
//! pinned by whitepaper §3.3.3 (post-amendment instance 31)
//! at `P128Pow5T3` over Pallas's base field with
//! `ConstantLength` domain (8 full + 56 partial rounds). A
//! cross-validation parity test against upstream
//! `halo2_poseidon 0.1.0` lives in the crate's `tests/`
//! directory to surface upstream drift as a development-time
//! signal.
...
//
// Adamant fork (Phase 6.8b.2): cfg gate widened from
// `feature = "test-dependencies"` to `any(test, feature =
// "test-dependencies")` so the Pow5Chip test in
// `pow5.rs::tests::poseidon_hash` can call it under
// `cargo test` without requiring the feature flag.
#[cfg(any(test, feature = "test-dependencies"))]
