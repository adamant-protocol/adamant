...
mod tests {
    ...
    /// MockProver triggers the EccChip's `configure`, which
    /// calls [`super::super::value_commitment_chip::RFullScalar::z`]
    /// + `u()`, which run `find_zs_and_us(R, 85)` on first
    /// use. That computation takes minutes the first run
    /// (lazy_static-cached thereafter). Until the
    /// pre-mainnet hardening sub-arc replaces runtime
    /// generation with hardcoded constants, this test (and
    /// its companions below) are `#[ignore]`-d so the
    /// default `cargo test` run stays fast. Run with
    /// `cargo test --release -- --ignored` to exercise.
    #[test]
    #[ignore = "triggers find_zs_and_us(R, NUM_WINDOWS); see module docs"]
    ...
    /// rejected.
    #[test]
    #[ignore = "triggers find_zs_and_us; see module docs"]
