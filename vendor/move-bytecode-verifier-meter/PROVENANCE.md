# Provenance: `move-bytecode-verifier-meter`
...
(Batch 2 of the Sui-Move vendoring). It provides the metering
trait surface (`Meter`, `Scope`, etc.) used by the verifier to
enforce resource limits during verification.

## Upstream

- **Project:** Sui (https://github.com/MystenLabs/sui)
- **Path within upstream repo:** `external-crates/move/crates/move-bytecode-verifier-meter`
...
- **`Cargo.toml`** — workspace-integration only. Specifically:
  ...
  - `description`: changed from upstream's
    `"Traits for metering the bytecode verifier"` to one pointing
    at this `PROVENANCE.md` and the whitepaper subsection.
...
Byte-identical to
`external-crates/move/crates/move-bytecode-verifier-meter/src/`
