...
(Batch 2 of the Sui-Move vendoring). It provides the
space-efficient abstract stack used by various verifier passes
(stack-effect tracking, type-safety, locals-safety, etc.).
...
- **`Cargo.toml`** — workspace-integration only. Specifically:
  ...
  - `authors`: kept as upstream's `["The Move Contributors"]`
    (matching Batch 1's `move-abstract-interpreter` pattern —
    crates already attributed to "The Move Contributors" upstream
    are not augmented; only crates with the older Diem-only
    attribution get the full-lineage augmentation).
  ...
  - `[lints]`: added per-crate `[lints.rust]` and `[lints.clippy]`
    (upstream had no `[lints]` section). The vendored code has
    no top-level `forbid(unsafe_code)` attribute upstream but is
...
Byte-identical to
`external-crates/move/crates/move-abstract-stack/src/` extracted
