...
provides the regex-based borrow graph used for reference safety on
regex types in the `regex_reference_safety` pass. At this scaffold
commit the workspace plumbing is in place; the actual upstream
source-file copy lands in the follow-up vendor commit per
`vendor/README.md`.
